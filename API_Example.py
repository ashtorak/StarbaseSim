...
class GameCommand(IntEnum):
    ...
    # "state": True/False (= Extend/Retract)
...
class RocketDataPacket:
    def __init__(self, objectname: str, location: List[float], rotation: List[float], velocity: List[float], fuelMass: float, oxidizerMass: float, enginesThatAreRunningBitmask: int, fuelGasMass: float, oxidizerGasMass: float, turbopumpTemperature: float, throttle: float, trajectoryHit: List[float]):
        ...
        self.fuelGasMass = fuelGasMass # gas mass in tons
        self.oxidizerGasMass = oxidizerGasMass # liquid mass in tons
        self.turbopumpTemperature = turbopumpTemperature # in Kelvin
        self.throttle = throttle # 0...1
        self.trajectoryHit = trajectoryHit # if trajectory prediction hits Z=0, this is the point, in m (+Y is East and +X is North)
        ...
        while '\n' in buffer:
            ...
            if (message and message != "Client still there?"):
                try:
                    ...
                    fuelGasMass = json_data.get('fuelGasMass', 0)
                    oxidizerGasMass = json_data.get('oxidizerGasMass', 0)
                    turbopumpTemperature = json_data.get('turbopumpTemperature', 0)
                    throttle = json_data.get('throttle', 0)
                    trajectoryHit =  json_data.get('trajectoryLastPoint', [])
                    ...
                    if isinstance(objectname, str) and isinstance(location, list) and len(location) == 3 and isinstance(rotation, list) and len(rotation) == 4:
                        RocketData.append(RocketDataPacket(objectname, location, rotation, velocity, fuelMass, oxidizerMass, enginesThatAreRunningBitmask, fuelGasMass, oxidizerGasMass, turbopumpTemperature, throttle, trajectoryHit))
                        
                        if(printSomeData):
                            # Extract state of Engine No. 3 (bit 3: left shift 1 by two)
                            is_engine_no3_running = (enginesThatAreRunningBitmask & (1 << 2)) != 0
                            print(f"Received transform: objectname = {objectname}\n location = {location}\n velocity = {velocity}\n propMass = {fuelMass+oxidizerMass}\n is engine No. 3 running: {is_engine_no3_running} \n fuelGas = {fuelGasMass}, oxiGas = {oxidizerGasMass}, turboTemp = {turbopumpTemperature}, throttle = {throttle} \n, trajectoryHit = {trajectoryHit} \n ")
                    else:
        ...
        try:
            ...
            if(test):
                ...
                command = {
                    ...
                    #"command": GameCommand.PadAOLMClampsExtend,
                    #"state": False

                    "command": GameCommand.PadASpawnStack
                }
                client.send((json.dumps(command) + "\n").encode())
