...
//#include "Physics/Experimental/PhysScene_Chaos.h" // For FPhysScene_Chaos
...
//
//void FSubsystemAsyncPhysicsTickCallback::OnPreSimulate_Internal()
//{
//	if (UAPISubsystem* SubsystemPtr = Subsystem.Get())
//	{
//		const float DeltaTime = GetDeltaTime_Internal();
//		UE_LOG(LogTemp, Verbose, TEXT("Custom callback executed, DeltaTime=%f"), DeltaTime);
//		SubsystemPtr->AsyncPhysicsTick(DeltaTime);
//	}
//	else
//	{
//		UE_LOG(LogTemp, Warning, TEXT("Subsystem is null in callback"));
//	}
//}
//
//void UAPISubsystem::Initialize(FSubsystemCollectionBase& Collection)
//{
...
//	{
//		UE_LOG(LogTemp, Log, TEXT("UAPISubsystem::Initialize called for World: %s, WorldType: %d, IsGameWorld: %d"),
//			*World->GetName(), (int32)World->WorldType, World->IsGameWorld());
//
...
//				this,
//				&UAPISubsystem::RegisterAsyncPhysicsCallback,
//				0.1f,
//				false
//			);
//			UE_LOG(LogTemp, Log, TEXT("Scheduled delayed physics callback registration"));
//		}
//		else
//		{
//			UE_LOG(LogTemp, Warning, TEXT("Skipping physics registration for non-game world (WorldType: %d)"), (int32)World->WorldType);
//		}
//	}
//
//	// TCP stuff
...
//}
//
//void UAPISubsystem::RegisterAsyncPhysicsCallback()
//{
//	if (UWorld* World = GetWorld())
//	{
...
//			UE_LOG(LogTemp, Log, TEXT("Physics Scene acquired. Solver: %s"), Solver ? TEXT("Valid") : TEXT("Null"));
//			if (Solver)
//			{
//				AsyncPhysicsCallback = Solver->CreateAndRegisterSimCallbackObject_External<FSubsystemAsyncPhysicsTickCallback>();
//				AsyncPhysicsCallback->SetSubsystem(this);
//				UE_LOG(LogTemp, Log, TEXT("Custom Async Physics Callback Registered"));
//			}
//			else
//			{
//				UE_LOG(LogTemp, Warning, TEXT("Failed to register callback: Solver is null"));
//			}
//		}
//		else
//		{
//			UE_LOG(LogTemp, Warning, TEXT("Failed to get Physics Scene in delayed registration"));
//		}
//	}
//}
//
//void UAPISubsystem::Deinitialize()
//{
//	// tick stuff
//
//	if (UWorld* World = GetWorld())
...
//		if (FPhysScene_Chaos* PhysScene = World->GetPhysicsScene())
//		{
//			if (AsyncPhysicsCallback)
//			{
//				PhysScene->GetSolver()->UnregisterAndFreeSimCallbackObject_External(AsyncPhysicsCallback);
//				AsyncPhysicsCallback = nullptr;
//				UE_LOG(LogTemp, Log, TEXT("Async Physics Callback Unregistered"));
//			}
//		}
//	}
//
...
//}
//
//void UAPISubsystem::AsyncPhysicsTick(float DeltaTime)
//{
...
//// all TCP stuff below
//
//void UAPISubsystem::SendData()
//{
//	if (GetWorld()->GetTimeSeconds() > LastTimeSendData + SendDataTick)
//	{
//		//for (TObjectPtr<ARocketActor> rocket : BoosterArray)
//		{
...
//					}
//				}
//				if (rocket) SendTransformData(rocket->IDstring, rocket->GetTransform());
//			}
...
//}
//
//void UAPISubsystem::HandleCommands()
//{
...
//}
//
//void UAPISubsystem::ProcessCommands(const FGameCommandData& CommandData, TObjectPtr<ARocketActor> ARocket)
//{
//	rocket = ARocket;
//	targetString = CommandData.target;
...
//		if (rocket) CPPCommand_ControllerAttitude(stateBool, parameterString);
//		break;
...
//		CPPCommand_Chopsticks(stateBool, valueFloat, parameterString);
//		break;
...
//}
//
//bool UAPISubsystem::InitializeServer()
//{
...
//}
//
//bool UAPISubsystem::AcceptClient()
//{
//	if (!ServerSocket)
//		return false;
//
...
//}
//
//void UAPISubsystem::SendTransformData(const FString& ObjectName, const FTransform& Transform)
//{
//	if (!ClientSocket || ClientSocket->GetConnectionState() != SCS_Connected)
//	{
...
//	JsonObject->SetStringField(TEXT("objectname"), ObjectName);
//
//	JsonObject->SetArrayField("location", TArray<TSharedPtr<FJsonValue>>{
//		MakeShareable(new FJsonValueNumber(Transform.GetLocation().X)),
//			MakeShareable(new FJsonValueNumber(Transform.GetLocation().Y)),
//			MakeShareable(new FJsonValueNumber(Transform.GetLocation().Z))
//	});
//
//	JsonObject->SetArrayField("rotation", TArray<TSharedPtr<FJsonValue>>{
//		MakeShareable(new FJsonValueNumber(Transform.GetRotation().X)),
//			MakeShareable(new FJsonValueNumber(Transform.GetRotation().Y)),
//			MakeShareable(new FJsonValueNumber(Transform.GetRotation().Z)),
//			MakeShareable(new FJsonValueNumber(Transform.GetRotation().W))
//	});
//
//	FString jsonString;
...
//}
//
//void UAPISubsystem::SendAndCheckConnection(FString JsonString)
//{
//	int32 BytesSent;
...
//}
//
//bool UAPISubsystem::ReceiveCommands(TArray<FGameCommandData>& OutCommands)
//{
//	OutCommands.Empty();
//	if (!ClientSocket || ClientSocket->GetConnectionState() != SCS_Connected)
//		return false;
...
//				if (CommandValue >= 0 && CommandValue <= static_cast<int32>(EGameCommand::MasseyDeluge))
