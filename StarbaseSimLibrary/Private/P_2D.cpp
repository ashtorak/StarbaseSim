...
FVector2f UP_2D::update_all(double& target_x, double& target_y, const FVector2d& measurement)
{
    // calculate distance _error
    _error = FVector2f(FVector2d{target_x, target_y} - measurement);

    // Constrain _error and target position
    // Constrain the maximum length of _vel_target to the maximum position correction velocity
    if (is_positive(_error_max) && limit_length(_error, _error_max)) {
...
void UP_2D::set_limits(float output_max, float D_Out_max, float D2_Out_max)
{
    _D1_max = 0.0f;
    _error_max = 0.0f;
...
void UP_2D::set_error_max(float error_max)
{
